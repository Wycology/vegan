...
specnumber(BCI)
```
Remember it is possible to change the names rownames from numbers to nglish letters as using `english` package as follows:
...
We can convert the `specnumber(BCI)` into a dataframe and arrange plots by the descending number of species.
...
It is estimating the number of species in the pool using `specpool()` function. There are 225 species, but the estimator gives a range of values from 236 to 247 with some standard errors around them. Would be interesting to go to the back-end and look at what goes on with this function.
