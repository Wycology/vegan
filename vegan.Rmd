...
We can convert the `specnumber(BCI)` into a df and arrange plots by the number of species.
...
It is estimating the number of species in the pool using `specpool()` function. There are 225 species, but the estimator gives values from 236 to 247 with some standard errors around them. Would be interesting to go to the back-end and look at what goes on with this function.
...
p3
```
These are further plots that can be created 
