...
The other way of getting the number of species per site in the data-set is to use `specnumber()` function within the `vegan` package.
...
It is possible to estimate the number of species in the pool using `specpool()` function in `R`. There are 225 species, but the estimator gives values from 236 to 247 with some standard errors around them.
