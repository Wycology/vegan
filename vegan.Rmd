---
title: "<center>**Introduction to vegan package in R**</center>"
...
In this write-up explore `vegan` package. `vegan` is an `R` library that features a number of c ecology relevant functions. Here I demonstrate some use case.

# Load data

Loading inbuilt data, BCI, that come with the `vegan` package.

```{r}
data("BCI")
```

Seems there ar a lot of data here, we can check the dimensions of the data frame using the `dim()` function from base `R` package.
...
```{r}
specnumber(BCI) |> as.data.frame() |> arrange(-specnumber(BCI))
```
...
Here is a dataset comprising 50 field plots indicating the number of individuals of species of trees counted in each sampling plot. Typical ecological survey work.

We can go ahead and calculate species diversity in the 50 plots using **shannon** index. There are also indices like **simpson** or **invsimpson** that one can calculate from the data-set.

## Shannon index

```{r}
diversity(BCI, index = 'shannon') # Just specify the index to be shannon
```

## Simpson index

```{r}
diversity(BCI, index = 'simpson') # Changing the code to calculate the simpson diversity index
```

## Inverse Simpson index

```{r}
diversity(BCI, index = 'invsimpson') # The code to run inverse of simpson index
```
...
```{r}
plot(diversity(BCI, index = 'shannon'), diversity(BCI, index = 'simpson')) # Kind of linear
```
