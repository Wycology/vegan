...
Here, I explore the `vegan` package. `vegan` is an R library that features in day to day working of a community ecologist. It would be great to know a few cases where the package comes in handy.

# Load data

We oad some inbuilt data, BCI, that come with the `vegan` package.
...
The other way of getting the number of species in the data-set is to use `specnumber()` function within the `vegan` package.

```{r}
specnumber(BCI)
```

Can have the specnumber(BCI) converted into a data frame and arranged to know which sampling plot had the highest number of species.

```{r}
specnumber(BCI) |> as.data.frame() |> arrange(-specnumber(BCI))
```

It is possible to estimate the number of species in the pool using `specpool()` function in `R`. Actually, there are 225 species, but the estimator gives values from 236 to 247 with some standard errors around them.
