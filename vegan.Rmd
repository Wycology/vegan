...
It is estimate the number of species in the pool using `specpool()` function from vegan in `R`. There are 225 species, but the estimator gives values from 236 to 247 with some standard errors around them. Would be interesting to go to the back-end and look at what goes on with this function.
...
Here we explore a data-set consisting of 50 field plots indicating the number of individuals of species of trees counted in each sampling plot. Typical ecological survey work.
