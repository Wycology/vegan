...
It is possible to estimate the number of species in the pool using `specpool()` function from vegan package in `R`. Actually, there are 225 species, but the estimator gives values from 236 to 247 with some standard errors around them.

```{r}
specpool(BCI, smallsample = TRUE)
```

# Demo calculating some selected diversity indices
...
```{r}
diversity(BCI, index = 'invsimpson') # Setting the code to run inverse of simpson index
```
