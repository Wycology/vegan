...
There are plenty data, we can check the dimension of the data frame using the `dim()` function from base `R` package.

```{r}
dim(BCI)
```

The other way of getting the number of species per plot in the data-set is to use `specnumber()` function within the `vegan` package.

```{r}
specnumber(BCI)
```

We can convert the `specnumber(BCI)` converted into df and arranged to show which sampling plot had the highest number of species and which had the least.
...
The indices are quite related to each other. This is because they as indicated in the following plots.
